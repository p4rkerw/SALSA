...
)**Step 0: Download tutorial dataset and align to your chosen reference with cellranger** We will download a a single cell gene expression dataset obtained from 1k PBMCs from a healthy donor and align it to our reference. This dataset uses the single cell gene expression v3 chemistry. If you want to skip ahead download the coordiante-sorted [bam](https://cf.10xgenomics.com/samples/cell-exp/6.0.0/1k_PBMCs_TotalSeq_B_3p_LT/1k_PBMCs_TotalSeq_B_3p_LT_possorted_genome_bam.bam) and [index](https://cf.10xgenomics.com/samples/cell-exp/6.0.0/1k_PBMCs_TotalSeq_B_3p_LT/1k_PBMCs_TotalSeq_B_3p_LT_possorted_genome_bam.bam.bai) from the 10X Genomics website.
```
...
```
# download to /mnt/g/reference/phasing/biallelic_SNV
...
```
head -n3 vcfdir/funcotation/pbmc.pass.rna.chr22hcphase.formatted.csv
