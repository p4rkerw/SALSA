...
**(Optional) Build a STAR index for 🌶️SALSA** If you are using a cellranger reference that was not built with STAR-2.5.1b you may want to build a new STAR reference. Alternatively, download the version of STAR that matches your existing cellranger reference and link the executable to /usr/bin/STAR . 
```
# runtime ~35min
STAR \
